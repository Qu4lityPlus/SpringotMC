...
import org.springotmc.commons.classpath.ClasspathResourceType;
import org.springotmc.commons.classpath.ClasspathScanner;
import org.springotmc.framework.SpringotFramework;
import org.springotmc.framework.annotation.*;
...
public class BeanManifest {
    ...
    private static final boolean DEBUG = Boolean.parseBoolean(System.getProperty("springot.framework.debug", "false"));
    ...
    public static BeanManifest of(@NonNull ClassLoader classLoader, @NonNull Class<?> clazz, @NonNull ComponentCreator creator, boolean fullLoad) {
        ...
        depends.addAll(Arrays.stream(clazz.getAnnotationsByType(SpringotApplication.class))
            .filter(Objects::nonNull)
    ...
    public static List<BeanManifest> of(@NonNull ClassLoader classLoader, @NonNull Class<?> parent, @NonNull SpringotApplication scan, @NonNull ComponentCreator creator) {
        ...
        List<BeanManifest> results = ClasspathScanner.of(classLoader)
            ...
            .map(resource -> {
                ...
                if (creator.isComponent(clazz) && !SpringotFramework.class.isAssignableFrom(clazz)) {
